...
namespace error {
	...
	static int count = 0;
	static std::vector<std::string> interpreter = {
	...
	static void print(std::ofstream& logs, long unsigned int error_code, int error_line, std::string bad_instruction) {
		error::count++;
	...
	static void print_status(std::ofstream& logs) {		
		if(error::count) {
